# Codex Dream Skin 桥接 Skill 使用指南
...
中文里的“子代理”可能指两种完全不同的机制：

- “子智能体”是临时、独立、通常用于并行工作的 agent；它的结果会回到
  当前主任务。
- “Codex 对话转交”是向另一个持久 Codex 对话发送信息；目标任务拥有
  自己的历史，可以稍后继续。
...
如果用户只说“发给子代理”，Skill 应先要求选择“子智能体（并行执行）”
或“Codex 对话转交（独立对话）”，不得根据当前可见窗口猜测。
...
## 路由二：Codex 对话转交（`codex-conversation`）
...
目标对话会拥有自己的历史和状态，适合长期回看、继续追问或在任务列表中
管理。它和临时子智能体的区别是“持久独立任务”，不是“并行 worker”。
...
```powershell
$root = 'C:\Users\HP\Documents\生图项目\workspace\Codex-Dream-Skin'
...
### 场景 D：把一个长期执行任务交给专门 Codex 对话

当前任务整理出实施 brief，使用 `codex-conversation` 创建或定位一个目标
Codex 对话。目标对话持续负责实现和回报，主对话保留决策上下文，两个任务
不会因为“当前可见窗口”变化而互相串线。
...
| 现象 | 正确处理 |
| --- | --- |
| 只说“子代理” | 停止发送，要求选择子智能体或 Codex 对话转交 |
...
| `windows/scripts/chatgpt-bridge.mjs` | Windows loopback CDP 桥接实现 |
| `windows/scripts/chatgpt-handoff-protocol.mjs` | handoff 严格解析和 checkpoint 逻辑 |
| `windows/tests/native-codex-bridge-skill-tests.mjs` | 四条路由、工具名和防混淆测试 |
| `windows/tests/run-tests.ps1` | 全量 Windows 回归入口 |

验证命令：

```powershell
python -X utf8 "$env:USERPROFILE\.codex\skills\.system\skill-creator\scripts\quick_validate.py" `
  'C:\Users\HP\Documents\生图项目\workspace\Codex-Dream-Skin\skills\dispatch-chatgpt-bridge'

node '.\windows\tests\native-codex-bridge-skill-tests.mjs'
powershell -NoProfile -ExecutionPolicy Bypass -File '.\windows\tests\run-tests.ps1'
```

权威 Skill 位于 `skills/dispatch-chatgpt-bridge/`，运行副本位于
`C:\Users\HP\.codex\skills\dispatch-chatgpt-bridge\`。两份内容同步后，
才算完成一次可复用的 Skill 更新。
