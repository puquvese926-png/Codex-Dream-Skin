...
const defaultStatePath = path.join(process.env.LOCALAPPDATA || "", "CodexDreamSkin", "state.json");
...
export function validateBridgeState(value) {
  if (!isPlainObject(value)) throw new Error("Dream Skin state must be an object");
  assertKnownFields(value, STATE_FIELDS, "Dream Skin state");
  if (value.schemaVersion !== 3) throw new Error("Dream Skin state schemaVersion must be 3");
  if (value.platform !== "windows") throw new Error("Dream Skin state platform must be windows");
  if (!Number.isInteger(value.port) || value.port < 1024 || value.port > 65535) {
    throw new Error("Dream Skin state port is invalid");
  }
  if (typeof value.browserId !== "string" || !CDP_ID_PATTERN.test(value.browserId)) {
    throw new Error("Dream Skin state browser identity is invalid");
  }
  for (const field of [
    "codexExe",
    "codexPackageFamilyName",
    ...
    if (typeof value[field] !== "string" || !value[field] || value[field].includes("\0")) {
      throw new Error(`Dream Skin state ${field} is invalid`);
    }
  }
